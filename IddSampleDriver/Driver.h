...
namespace Microsoft
{
    namespace IndirectDisp
    {
        ...
        public:
            ...
            NTSTATUS ConnectMonitor();
            NTSTATUS DisconnectMonitor();
            NTSTATUS ReconnectMonitor();   // Disconnect + reconnect at new s_Desired* resolution
            bool IsMonitorConnected() const { return m_MonitorArrived; }

        protected:
            WDFDEVICE     m_WdfDevice;
            IDDCX_ADAPTER m_Adapter;
            IDDCX_MONITOR m_Monitor = nullptr;  // Current virtual monitor (nullptr if none)
            bool          m_MonitorArrived = false;
        };
